...
// TestSuspendJobStartTimeValidation verifies that the job controller can update startTime
// when resuming a suspended job (https://github.com/kubernetes/kubernetes/issues/134521).
func TestSuspendJobStartTimeValidation(t *testing.T) {
	...
	if getJobConditionStatus(ctx, job, batchv1.JobSuspended) != v1.ConditionFalse {
		t.Fatalf("JobSuspended condition was not set to False")
	}
	if job.Status.StartTime == nil {
		t.Fatalf("Job startTime was not set after resume")
	}
}
